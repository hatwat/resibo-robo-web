...
function StatusPill({ status }) {
  ...
  return (
    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold border ${style.color} font-display`}>
      {style.label}
    </span>
  )
}
...
export default function PendingInvoiceCard({ invoice, session, onInvoiceRemoved }) {
  ...
  const getAuthToken = async () => {
    const { data: { session: currentSession } } = await import('../lib/supabaseClient').then(m => m.supabase.auth.getSession())
    return session?.access_token || currentSession?.access_token
  }
  ...
  return (
    ...
    >
      {/* Header */}
      <div className="flex items-center justify-between px-6 py-4 border-b border-ink-100 bg-ink-900">
        <div className="flex items-center gap-3">
          <div>
            <p className="font-display font-semibold text-white text-sm leading-none">
              {formData.vendor_name || 'Unknown Vendor'}
            </p>
            <p className="font-mono text-ink-500 text-xs mt-0.5">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-ink-500 text-xs font-body">{timeAgo}</span>
        </div>
      </div>
      ...
      {/* Body: split 2 columns on large screens */}
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <input
                  ...
                  disabled={isWorking}
                  className="field-input font-display font-bold text-base text-accent-green"
                />
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 p-6">
            ...
            {driveUrl && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="mt-3 flex items-center justify-center gap-1.5 text-xs text-ink-400 hover:text-accent-blue transition-colors font-body"
              >
          ...
          {formData.validation && (
            <div className="px-6 pb-4">
              <div className={`rounded-xl p-3 text-xs font-body border ${
                ...
                <p className="font-semibold font-display mb-1">AI Validation: {validationStatus}</p>
